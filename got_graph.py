...
class GotGraph(object):
    ...
    def __init__(self,  auth=('neo4j', 'Aa0861029'), host='localhost', port=7687, secure=False, ):
    ...
    def get_character(self, character_id):
        ...
        if n is not None and len(n) > 0:
            ...
            print("QQ")
            n = None
        print("HIHIHIHIIHIHHI" , type(n))
        return n

    def get_related_characters(self, ch_id, r_kind):

        t = self.get_character(ch_id)
        print("all: ", dict(t))
        print("relationship: ",r_kind)
        nodes = set()
        nodes.add(t)
        #print(ch_id, r_kind)
        n = self._relationship_matcher.match(nodes, r_type=r_kind)
        return n
